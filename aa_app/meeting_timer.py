  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .bar-wrap.has-reminder-frame {
      border-color: #ff9d2f;
      box-shadow: 0 0 0 4px rgba(255, 157, 47, 0.96), 0 0 0 9px rgba(255, 157, 47, 0.24);
    }
  ...
  <main class="app">
    ...
    <details>
      <summary>
        ...
        <span id="settingsHint">Preset og custom tímar</span>
      </summary>
      <div class="settings">
        ...
        <div class="settings-card">
          ...
          <div class="settings-actions single">
            <button type="button" class="settings-button" id="resetPresetsButton">Endurheimta sjálfgefin preset</button>
          </div>
        </div>
    ...
    (() => {
      ...
      function statusText() {
        ...
        if (state.deadlineAt === null && state.remainingSeconds === state.totalSeconds) {
          return 'Veldu preset og ýttu á Byrja.';
        }
        if (state.deadlineAt === null) {
          return 'Timer í pásu';
        }
